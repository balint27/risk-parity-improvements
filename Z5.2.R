#=============================================================================
# ENHANCED RISK PARITY TRADING SYSTEM - VERSION Z5.2.R
...
create_market_data <- function(prices, returns = NULL, enhanced_indicators = TRUE) {
  ...
  # 8. NEW: Yield curve indicator (if available)
  if (all(c("SHY", "IEF", "TLT") %in% colnames(prices))) {
  ...
  # 9. NEW: Market breadth indicator (if available with high/low data)
...
calculate_cash_allocation <- function(current_drawdown, max_drawdown, vol_zscore, regime, max_cash_pct = 0.50) {
  # More aggressive cash allocation
  cash_pct <- 0
  
  # Start increasing cash earlier - at 40% of max drawdown instead of 60%
  if (current_drawdown > 0.4 * max_drawdown) {
    dd_ratio <- current_drawdown / max_drawdown
    dd_cash_pct <- min(max_cash_pct, (dd_ratio - 0.4) * 2.5 * max_cash_pct)
    cash_pct <- max(cash_pct, dd_cash_pct)
  }
  
  # More aggressive vol-based cash allocation
  if (vol_zscore > 0.75) {  # Lower threshold (was 1.0)
    vol_cash_pct <- min(max_cash_pct, (vol_zscore - 0.75) * 0.15)
    cash_pct <- max(cash_pct, vol_cash_pct)
  }
  
  # Add regime-based cash allocation
  if (!is.null(regime) && !is.na(regime) && regime != "") {
    if (regime %in% c("risk_off", "deflation", "stagflation")) {
      regime_cash <- ifelse(regime == "risk_off", 0.30, 
                            ifelse(regime == "deflation", 0.20, 0.15))
...
#=============================================================================
# ENHANCED REGIME DETECTION WITH IMPROVED SENSITIVITY
#=============================================================================
...
detect_market_regime <- function(market_data, lookback = 252, default_regime = "growth") {
  cat("\n---- ENHANCED REGIME DETECTION ----\n")
  
  # Safety check
  if (is.null(market_data) || nrow(market_data) < 60) { # Reduced minimum requirement
  ...
  # IMPORTANT FIX: Only require lookback/2 days instead of full lookback
  if (nrow(market_data) <= lookback/2) {
    # If we don't have enough data, use all but the latest point
    historical_data <- head(market_data, nrow(market_data) - 1)
  } else {
    # Otherwise use half the lookback window
    historical_data <- head(tail(market_data, ceiling(lookback/2)), ceiling(lookback/2) - 1)
  }
  ...
  regime_probs <- list(
    growth = 0.05,       # Minimum probability floor
    reflation = 0.01,
    deflation = 0.01,
    stagflation = 0.01,
    risk_off = 0.01
  ...
  # Define threshold for binary classification - LOWERED from 0.5 to 0.3
  threshold <- 0.3
  ...
  if (vol_signal) {
    regime_probs$risk_off = 0.5 + min((z_scores$volatility - threshold) * 0.15, 0.4)
    ...
    regime_probs$risk_off = max(0.01, (z_scores$volatility - 0) * 0.25)
  }
  ...
  if (growth_signal && !inflation_signal && !vol_signal) {
    ...
    regime_probs$growth = max(0.05, 0.15 - abs(z_scores$growth) * 0.1)
  }
  
  # 3. Reflation regime (positive growth AND rising inflation)
  if (growth_signal && inflation_signal && !vol_signal) {
    regime_probs$reflation = 0.5 + 
      min((z_scores$growth + z_scores$inflation) * 0.075, 0.4)
  } else if (z_scores$growth > 0 && z_scores$inflation > 0) {
    regime_probs$reflation = max(0.01, z_scores$growth * 0.25 + z_scores$inflation * 0.25)
  } else {
    regime_probs$reflation = 0.01
  }
  ...
  if (growth_neg_signal && inflation_neg_signal) {
    ...
    regime_probs$deflation = 0.5 + 
      min((abs(z_scores$growth) + abs(z_scores$inflation)) * 0.075, 0.4)
    ...
    if (!is.null(z_scores$credit) && z_scores$credit < -0.3) { # More sensitive threshold
      regime_probs$deflation = min(0.9, regime_probs$deflation + 0.15)
    }
  }
  ...
  if (growth_neg_signal && inflation_signal) {
    regime_probs$stagflation = 0.5 + 
      min((abs(z_scores$growth) + z_scores$inflation) * 0.075, 0.4)
  } else if (z_scores$growth < 0 && z_scores$inflation > 0) {
    regime_probs$stagflation = max(0.01, abs(z_scores$growth) * 0.25 + z_scores$inflation * 0.25)
  }
  
  # If no clear regime detected, increase default regime probability
  if (max(unlist(regime_probs)) < 0.3) {
    regime_probs[[default_regime]] <- max(regime_probs[[default_regime]], 0.3)
  }
...
# FIXED: Map asset class weights to ETF weights with better error handling
...
optimize_for_transaction_costs <- function(prev_weights, current_weights, transaction_costs,
  ...
  if (!is.null(regime) && !is.na(regime) && regime %in% c("risk_off", "deflation")) {
...
calculate_portfolio_performance <- function(
    ...
    transaction_costs = NULL, max_cash_pct = 0.50) {
  ...
  if (track_regimes && !is.null(market_data)) {
    ...
    # Always create regime_history to avoid errors
    regime_history <- xts(
      matrix("", nrow = nrow(prices), ncol = 1), 
      order.by = index(prices)
    )
  ...
  total_turnover <- 0
  total_cost <- 0
  current_regime <- ""  # Track current regime
  ...
  for (i in 1:nrow(prices)) {
    ...
    if (date %in% rebalance_dates || i == 1) {
      ...
      if (track_regimes && !is.null(market_data)) {
        ...
        # MAJOR FIX: Remove minimum data requirement that was causing empty regimes
        # Only require at least 60 days of data instead of full lookback window
        if (nrow(market_subset) >= 60) {
          ...
          cat("  Insufficient data for regime detection, using default regime\n")
      ...
      if (!is.null(prev_weights)) {
        ...
        current_weights <- optimize_for_transaction_costs(
          ...
          current_regime, vol_zscore, 0.20  # Lower base turnover threshold (20%)
        )
      }
      ...
      if (i > 1) {
        ...
        current_cash <- calculate_cash_allocation(
          ...
          regime = current_regime,  # Now includes regime
  ...
  results <- list(
    ...
    regime_history = regime_history  # Always include regime history
  )
...
run_enhanced_backtest <- function(
    ...
    min_weight = 0.01,
    max_weight = 0.30,
    max_cash = 0.50,  # Increased from 0.25 to 0.50
    include_inverse = TRUE) {
  ...
  results <- calculate_portfolio_performance(
    ...
    track_regimes = TRUE,  # Always track regimes
...
min_weight <- 0.01
max_weight <- 0.30
max_cash <- 0.50  # Increased maximum cash allocation to 50%
...
cat("Thank you for using the Enhanced Risk Parity System Z5.2.R\n")