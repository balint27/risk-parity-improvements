#=============================================================================
# ENHANCED RISK PARITY TRADING SYSTEM - VERSION Z5.5.R
...
optimize_risk_parity <- function(cov_matrix, custom_risk_targets = NULL, 
  ...
  tryCatch({
    ...
    # FIXED VERSION: Use correct interface for nloptr
    result <- nloptr::slsqp(
      x0 = equal_weights,
      ...
      lower = rep(0.001, n),
      upper = rep(0.999, n),
      hin = function(w) sum(w) - 1,  # Sum of weights = 1 constraint
      hin.jac = function(w) rep(1, length(w)),
      control = list(
...
construct_portfolio <- function(weights, prices, target_volatility = 0.10, 
  ...
  if (is.xts(prices)) {
    ...
    # FIX #1: Add this code to ensure proper Date format
...
# Fix the specific issue in calculate_portfolio_stats function where
# matrix multiplication drops the xts class
...
#=============================================================================
# PART 6: MAIN EXECUTION FUNCTION WITH ERROR HANDLING AND PERFORMANCE ANALYSIS
#=============================================================================
...
# Fixed version of generate_performance_report with proper error handling
generate_performance_report <- function(strategy_results, returns, prices, 
  ...
  if (length(common_assets) > 0) {
    ...
    # FIXED: Calculate portfolio returns using rowSums to maintain xts structure
    ...
    if (nrow(port_returns) >= lookback_periods$medium) {
      # Use tryCatch for the entire risk metrics calculation
      tryCatch({
        ...
        # Volatility - with safety checks
        vol_port <- max(0.0001, sd(analysis_period[, 1], na.rm = TRUE)) * sqrt(252)
        vol_bench <- max(0.0001, sd(analysis_period[, 2], na.rm = TRUE)) * sqrt(252)
        
        # Sharpe ratio (with error handling)
        sharpe_port <- ifelse(vol_port > 0 && is.finite(annual_port/vol_port), 
                              annual_port/vol_port, NA)
        sharpe_bench <- ifelse(vol_bench > 0 && is.finite(annual_bench/vol_bench), 
                               annual_bench/vol_bench, NA)
        
        # Calculate drawdowns safely
        dd_port <- tryCatch({
          as.numeric(maxDrawdown(analysis_period[, 1]))
        }, error = function(e) NA)
        
        dd_bench <- tryCatch({
          as.numeric(maxDrawdown(analysis_period[, 2]))
        }, error = function(e) NA)
        ...
        # Only print Sharpe if valid
        if (!is.na(sharpe_port) && !is.na(sharpe_bench)) {
          cat(sprintf("  Sharpe Ratio: Portfolio = %.2f, %s = %.2f\n", 
                      sharpe_port, benchmark_name, sharpe_bench))
        }
        
        # Only print drawdown if valid
        if (!is.na(dd_port) && !is.na(dd_bench)) {
          cat(sprintf("  Maximum Drawdown: Portfolio = %.2f%%, %s = %.2f%%\n", 
                      dd_port * 100, benchmark_name, dd_bench * 100))
        }
        
        # Store metrics for return (with proper error checking)
        metrics <- list(
          returns = port_returns,
          cumulative_returns = merge(cumprod(1 + port_returns[, 1]), cumprod(1 + port_returns[, 2])),
          drawdowns = if(!is.na(dd_port) && !is.na(dd_bench)) {
            merge(
              (cumprod(1 + port_returns[, 1]) / cummax(cumprod(1 + port_returns[, 1])) - 1) * 100,
              (cumprod(1 + port_returns[, 2]) / cummax(cumprod(1 + port_returns[, 2])) - 1) * 100
            )
          } else { NULL },
        ...
        cat("\nWarning: Error calculating risk metrics:", e$message, "\n")
        # Return basic metrics if more advanced calculations fail
        metrics <- list(
          returns = port_returns,
          cumulative_returns = merge(cumprod(1 + port_returns[, 1]), cumprod(1 + port_returns[, 2]))
        )
      ...
      # Current drawdown status - with robust error handling
      tryCatch({
        ...
        # Make sure we have valid values before printing
        last_port_dd <- as.numeric(tail(port_dd, 1))
        last_bench_dd <- as.numeric(tail(bench_dd, 1))
        
        if (is.finite(last_port_dd) && is.finite(last_bench_dd)) {
          cat(sprintf("\nCurrent Drawdown: Portfolio = %.2f%%, %s = %.2f%%\n", 
                      last_port_dd, benchmark_name, last_bench_dd))
        }
        
        # Save drawdown data to metrics
        if (exists("metrics")) {
          metrics$drawdowns <- merge(port_dd, bench_dd)
        }
      }, error = function(e) {
        cat("\nWarning: Error calculating current drawdown:", e$message, "\n")
      })
      
      # Return calculated metrics if they exist
      if (exists("metrics")) {
        return(metrics)
      }
    }
    
    # Basic metrics if we don't have enough data
    return(list(
      returns = port_returns,
      cumulative_returns = merge(cumprod(1 + port_returns[, 1]), cumprod(1 + port_returns[, 2]))
    ))
...
# Fixed plot_portfolio_allocation function with robust error handling
plot_portfolio_allocation <- function(portfolio, exposures = NULL) {
  ...
  # Plot 2: Risk contribution if available - WITH ROBUST ERROR HANDLING
  p2 <- tryCatch({
    if (!is.null(portfolio$risk_contribution) && length(portfolio$risk_contribution) > 0) {
      ...
      ggplot(risk_df, aes(x = Asset, y = Contribution, fill = Asset)) +
        geom_bar(stat = "identity") +
        scale_y_continuous(labels = scales::percent) +
      ...
      # Create a placeholder plot if risk contribution not available
      ggplot() + 
        annotate("text", x = 0.5, y = 0.5, label = "No risk contribution data available") +
        theme_void()
    }
  }, error = function(e) {
    ...
    ggplot() + 
      annotate("text", x = 0.5, y = 0.5, 
               label = paste("Error creating risk plot:", e$message)) +
      theme_void()
  })
  
  # Plot 3: Asset class exposure if available - WITH ROBUST ERROR HANDLING
  p3 <- tryCatch({
    if (!is.null(exposures) && !is.null(exposures$asset_class) && length(exposures$asset_class) > 0) {
      ...
      ggplot(class_df, aes(x = "", y = Exposure, fill = Class)) +
      ...
      ggplot() + 
        annotate("text", x = 0.5, y = 0.5, label = "No asset class data available") +
        theme_void()
    }
  }, error = function(e) {
    ggplot() + 
      annotate("text", x = 0.5, y = 0.5, 
               label = paste("Error creating asset class plot:", e$message)) +
      theme_void()
  })
  
  # Plot 4: Factor exposure if available - WITH ROBUST ERROR HANDLING
  p4 <- tryCatch({
    if (!is.null(exposures) && !is.null(exposures$factor) && length(exposures$factor) > 0) {
      factor_exposure <- unlist(exposures$factor)
      
      # Only proceed if we have valid factor exposures
      if (length(factor_exposure) > 0) {
        ...
        ggplot(factor_df, aes(x = Factor, y = Exposure, fill = Exposure > 0)) +
          geom_bar(stat = "identity") +
        ...
        ggplot() + 
          annotate("text", x = 0.5, y = 0.5, label = "Empty factor exposure data") +
          theme_void()
      }
    } else {
      # Empty plot if factor exposures not available
      ggplot() + 
        annotate("text", x = 0.5, y = 0.5, label = "No factor exposure data available") +
        theme_void()
    }
  }, error = function(e) {
    ggplot() + 
      annotate("text", x = 0.5, y = 0.5, 
               label = paste("Error creating factor plot:", e$message)) +
      theme_void()
  })
  ...
  grid_plot <- tryCatch({
    ...
    # If grid.arrange fails, return just the weight plot
    cat("Error combining plots:", e$message, "\n")
    cat("Displaying only portfolio weights\n")
    return(p1)
  })
  
  return(grid_plot)
}
# Main execution function with fixed order of operations
run_risk_parity_strategy <- function(tickers = c("SPY", "QQQ", "IWM", "EFA", "EEM", 
  ...
  # Print header
  cat("\n========================================================\n")
  cat("ENHANCED RISK PARITY TRADING SYSTEM - VERSION Z5.6\n")
  cat("========================================================\n")
  cat(sprintf("Running with simulation date: %s\n", format(end_date, "%Y-%m-%d")))
  ...
  tryCatch({
    ...
    # Fix date formats
    index(prices) <- as.Date(format(index(prices), "%Y-%m-%d"))
    ...
    # Fix date formats
    index(market_data) <- as.Date(format(index(market_data), "%Y-%m-%d"))
    ...
    # Step 11: Calculate portfolio statistics (ONLY APPEARS AFTER PORTFOLIO IS CONSTRUCTED)
    # Fix portfolio dates before calculating statistics
    portfolio <- fix_portfolio_calculation(portfolio, returns)
...
# Main function for complete analysis with performance metrics and plots
...
# Set system date and user information for simulation
Sys.setenv(TZ = "UTC")  # Set timezone to UTC
simulation_date <- as.Date("2025-09-04")  # Use date provided by user
current_user <- "balint27continues"       # Use username provided by user
