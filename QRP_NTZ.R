###############################################################################
# Unified Risk Parity Engine & Professional Assessment Pack Generator         #
#                                                                             #
# VERSION:   v160.9.20 (QRPv18-TypeHarden)                                    #
# DATE:      2025-11-14 12:46:29 UTC                                          #
# AUTHOR:    balint27                                                         #
#                                                                             #
# CHANGELOG:                                                                  #
# v160.9.20: - CRITICAL BUGFIX (Type/Shape Mismatch): Patched a fatal error   #
#              where 1xN matrices (from xts rows or t()) were passed to an   #
#              aligner that expected named vectors.                          #
#            - Hardened `.align_weights_to_universe` to be type-aware,       #
#              correctly handling vectors, matrices, and xts rows by reading #
#              `names()`, `colnames()`, or `rownames()`.                       #
#            - Changed delta storage in `raw_trade_log` to explicitly create #
#              a 1xN matrix with `colnames`, ensuring shape integrity.       #
#                                                                             #
# v160.9.18: - IMPLEMENTED: No-Trade Zone (NTZ) logic in the main backtest    #
#              loop to suppress micro-trades and reduce turnover.            #
#                                                                             #
###############################################################################
...
# ============================== HELPERS (v160.9.20 - Hardened) ===============================
`%||%` <- function(a, b) if (is.null(a)) b else a

# --- Canonical CAGR helpers ---
.calc_years_calendar <- function(R) {
  as.numeric(as.Date(end(R)) - as.Date(start(R))) / 365.25
}

# Calendar-time CAGR: matches finance convention for TWR across arbitrary windows
cagr_calendar <- function(R) {
  if (NROW(R) < 2) return(NA_real_)
  TR <- prod(1 + R, na.rm = TRUE) - 1
  yrs <- .calc_years_calendar(R)
  if (!is.finite(yrs) || yrs <= 0) return(NA_real_)
  (1 + TR)^(1 / yrs) - 1
}

# If you prefer PA-style annualization but want it to track calendar time,
# compute an effective periods-per-year from the data and pass it through.
cagr_pa_effective <- function(R) {
  if (NROW(R) < 2) return(NA_real_)
  dpy_eff <- NROW(R) / .calc_years_calendar(R)  # realized periods per calendar year
  PerformanceAnalytics::Return.annualized(R, geometric = TRUE, scale = dpy_eff)
}
...
# --- FIX: Type-aware aligner from v160.9.20 ---
.align_weights_to_universe <- function(w, universe) {
  stopifnot(!is.null(universe), length(universe) > 0)
  
  # Extract "names" depending on storage type
  nms <- names(w)
  if (is.null(nms) && is.matrix(w)) {
    nms <- colnames(w)
    if (is.null(nms) && ncol(w) == 1L) nms <- rownames(w)  # column-vector case
  }
  if (is.null(nms) && xts::is.xts(w)) nms <- colnames(w)
  
  # If we can identify names, align and return a full-named numeric vector
  if (!is.null(nms)) {
    out <- setNames(numeric(length(universe)), universe)
    common <- intersect(nms, universe)
    if (length(common)) {
      if (is.matrix(w) || xts::is.xts(w)) {
        if (NROW(w) == 1L && !is.null(colnames(w))) {
          vv <- as.numeric(w[1, common, drop = TRUE]); names(vv) <- common
        } else if (NCOL(w) == 1L && !is.null(rownames(w))) {
          vv <- as.numeric(w[common, 1, drop = TRUE]); names(vv) <- common
        } else {
          vv <- as.numeric(w[common]); names(vv) <- common
        }
      } else {
        vv <- as.numeric(w[common]); names(vv) <- common
      }
  ...
  # Truly unnamed, non-empty input -> error
  if (!is.null(w) && length(w) > 0) {
    stop(".align_weights_to_universe: input has no names (vector), colnames (matrix/xts), or rownames (col-matrix)")
  }
  
  # Empty input => zero vector over the universe
  setNames(numeric(length(universe)), universe)
}
...
calculate_target_weights <- function(prices, R, rebal_idx, previous_weights, master_controls, vix_series, trend_signals) {
  ...
  if (NROW(na.omit(r_win_full)) < 25) {
    ...
    return(list(target_weights = target_weights, log_entry = log_entry, diag_data = diag_data, audit_df = NULL, vix_event_log = NULL, asset_annual_vol = setNames(numeric(0), character(0)), eligible_assets = character(0)))
  }
  ...
  if (length(eligible_assets) < 1) {
    ...
    diag_data <- list(ex_ante_vol = x_vol, leverage = x_lev, gross_exposure = x_gross, avg_corr = x_corr)
    return(list(target_weights = target_weights, log_entry = data.frame(Date=target_start_date, Type="DE-RISK", Details="Not enough eligible assets after filtering."), diag_data = diag_data, audit_df = NULL, vix_event_log = NULL, asset_annual_vol = setNames(numeric(0), character(0)), eligible_assets = character(0)))
  }
  ...
  w_scaled_vec_aligned <- .align_weights_to_universe(w_scaled_vec, all_cols)
  target_weights <- make_row_xts_strict(w_scaled_vec_aligned, all_cols, target_start_date, name="target_weights_final")
  ...
  audit_df <- NULL
  asset_vols <- setNames(numeric(0), character(0))
  ...
  return(list(target_weights = target_weights, log_entry = log_entry, diag_data = diag_data, audit_df = audit_df, vix_event_log = vix_event_log, asset_annual_vol = asset_vols, eligible_assets = eligible_assets))
}
...
risk_parity_backtest <- function(prices, master_controls) {
  ...
  universe <- colnames(R) # Define universe once
  
  vix_series <- get_vix_aligned(index(prices), start(prices), end(prices))
  class_map_for_trend <- .default_class_map(mc$ytd_enhancements$new_asset_classes)
  trend_assets_to_use <- intersect(colnames(prices), names(class_map_for_trend))
  ...
  # --- FIX: Ensure initial delta matrix has colnames ---
  initial_delta_mat <- matrix(w_initial[universe], nrow = 1, dimnames = list(NULL, universe))
  raw_trade_log[[1]] <- list(date = index(R)[1], reason = "initialization", deltas = initial_delta_mat, w_after = w_initial)
  ...
  for (i in 2:n_days) {
    ...
    if (today %in% rebal_dates) {
      ...
      if (length(signal_calc_idx) > 0) {
        ...
        if (!is.null(calc_result)) {
          w_target_vec <- .normalize_w(calc_result$target_weights, universe)
          names(w_target_vec) <- universe
          w_after_vec <- w_target_vec # Default case: trade to target
          
          # --- NO-TRADE ZONE (NTZ) LOGIC - v160.9.18 ---
          if (isTRUE(mc$no_trade_bands$enabled)) {
            ntz <- mc$no_trade_bands
            class_map <- .default_class_map(mc$ytd_enhancements$new_asset_classes)
            
            # 1. Calculate per-asset tolerance bands
            asset_vols <- calc_result$asset_annual_vol
            sigma_aligned <- .align_weights_to_universe(asset_vols, universe)
            
            band <- ntz$alpha * (sigma_aligned / (ntz$sigma_ref %||% 0.15))
            band <- pmin(pmax(band, ntz$min_abs), ntz$max_abs)
            
            # 2. Enforce exemptions and hard rules
            exempt_assets <- names(w_current)[class_map[names(w_current)] %in% ntz$exempt_classes]
            band[exempt_assets] <- 0
            
            if (isTRUE(ntz$respect_hard_rules)) {
              hard_rule_assets <- setdiff(names(w_current), calc_result$eligible_assets)
              band[hard_rule_assets] <- 0
            }
            
            # 3. Compare levered target vs current drifted weights
            w_current_aligned <- .align_weights_to_universe(w_current, universe)
            breach <- abs(w_target_vec - w_current_aligned) > band
            
            # 4. Build execution weights: start with current, overwrite breaches
            w_exec <- w_current_aligned 
            
            if (any(breach)) {
              if (identical(ntz$snap_mode, "target")) {
                w_exec[breach] <- w_target_vec[breach]
              } else { # "band_edge"
                over <- (w_current_aligned > w_target_vec) & breach
                under <- (w_current_aligned < w_target_vec) & breach
                w_exec[over]  <- w_target_vec[over]  + band[over]
                w_exec[under] <- w_target_vec[under] - band[under]
              }
            }
            
            # 5. Adjust cash only to keep row sum at 1
            risky_sum_exec <- sum(w_exec[setdiff(names(w_exec), cash_asset)], na.rm = TRUE)
            w_exec[cash_asset] <- 1 - risky_sum_exec
            w_after_vec <- w_exec # This is now the actual target for execution
            
            suppressed_count <- sum(!breach & names(breach) != cash_asset)
            total_risk_assets <- length(setdiff(universe, cash_asset))
            message(sprintf("[NTZ %s] Suppressed %d/%d trades.", today, suppressed_count, total_risk_assets))
          ...
          w_current_aligned <- .align_weights_to_universe(w_current, universe)
          w_after_vec_aligned <- .align_weights_to_universe(w_after_vec, universe)
          
          delta_vec <- w_after_vec_aligned - w_current_aligned
          
          # --- FIX: Store deltas as a 1xN matrix with colnames ---
          deltas_1xN <- matrix(delta_vec[universe], nrow = 1, dimnames = list(NULL, universe))
          raw_trade_log[[length(raw_trade_log) + 1]] <- list(date = today, reason = "rebalance", deltas = deltas_1xN, w_after = w_after_vec_aligned)
          w_current <- w_after_vec_aligned
          
          # --- WEIGHT SUM SANITY CHECK ---
          s <- sum(w_current)
          if (abs(s - 1) > 1e-8) {
            warning(sprintf("[WEIGHT SUM DRIFT] %s sum=%.8f (fixing via cash)", as.character(today), s))
            if (cash_asset %in% names(w_current)) {
              w_current[cash_asset] <- w_current[cash_asset] + (1 - s)
            } else {
              w_current <- w_current / s # Fallback
            }
          }
    ...
    if (isTRUE(mc$stop_loss$enabled)) {
      ...
      if (!is.na(drawdown) && drawdown > mc$stop_loss$thresholds_by_class$default) {
        ...
        w_current_aligned <- .align_weights_to_universe(w_current, universe)
        delta_sl <- w_current_after_sl - w_current_aligned
        
        # --- FIX: Store deltas as a 1xN matrix with colnames ---
        deltas_sl_1xN <- matrix(delta_sl[universe], nrow = 1, dimnames = list(NULL, universe))
        raw_trade_log[[length(raw_trade_log) + 1]] <- list(date = today, reason = "stop-loss", deltas = deltas_sl_1xN, w_after = w_current_after_sl)
  ...
  if (length(raw_trade_log) > 0) {
    ...
    net_mat <- do.call(rbind, lapply(by_date, function(idxs) {
      deltas_list <- lapply(raw_trade_log[idxs], `[[`, "deltas")
      # --- The hardened aligner now correctly handles matrix inputs from the log ---
      aligned_deltas <- lapply(deltas_list, function(d) .align_weights_to_universe(d, universe))
      Reduce(`+`, aligned_deltas)
    }))
  ...
  for (i in 2:NROW(R)) {
    ...
    # --- The hardened aligner now correctly handles w_current from the lookup ---
    positions_mat[i, ] <- .align_weights_to_universe(w_current, universe)
  }
...
calculate_rpqs_granular <- function(data_artifact, analysis_periods) {
  ...
  calculate_metrics_for_period <- function(perf_range, results) {
    ...
    perf_cagr <- cagr_calendar(perf_returns) # <-- FIX: Use canonical CAGR
...
# ============================ DIAGNOSTICS & REPORTING (v160.9.14) ====================================
print_console_summary <- function(data_artifact, analysis_periods) {
  ...
  for (period_name in names(enabled_periods)) {
    ...
    cat(sprintf("Start: %s | End: %s | Avg. Ann. Return (CAGR): %.2f%% | Annual Vol: %.2f%% | Sharpe Ratio: %.2f | Max Drawdown: %.2f%%\n",
                ...
                as.character(end(Rp)),
                cagr_calendar(Rp)*100, # <-- FIX: Use canonical CAGR
...
generate_professional_assessment_pack <- function(data_artifact, analysis_periods) {
  ...
  # --- CANONICAL FULL PERIOD CALCULATION (SINGLE SOURCE OF TRUTH) ---
  message(sprintf("[W1] 01/03 window: %s / %s", analysis_start_date, end(results$portfolio_returns_xts)))
  
  Rp_full_window <- results$portfolio_returns_xts[paste0(analysis_start_date, "/", end(results$portfolio_returns_xts))]
  TR_can <- prod(1 + Rp_full_window, na.rm = TRUE) - 1
  CAGR_can <- cagr_calendar(Rp_full_window)
  
  message(sprintf("[W2] Canonical CAGR (daily, calendar)= %.6f", CAGR_can))
  # --- END CANONICAL BLOCK ---
  ...
  addWorksheet(wb, "01_Run_Summary"); tryCatch({
    ...
    summary_list <- list()
    all_periods_with_full <- c(enabled_periods, list("Full Period" = list(start=analysis_start_date, end=end(portfolio_returns_xts), enabled=TRUE)))
    ...
    for (period_name in names(all_periods_with_full)) {
      ...
      # Use canonical values for the "Full Period" row
      net_cagr_val <- if (period_name == "Full Period") CAGR_can else cagr_calendar(Rp_net)
      net_tr_val <- if (period_name == "Full Period") TR_can else (prod(1 + Rp_net, na.rm = TRUE) - 1)
      ...
      summary_list[[period_name]] <- c(
        ...
        Net_Total_Return_Pct = net_tr_val * 100,
        Gross_CAGR_Pct = cagr_calendar(Rp_gross)*100,
    ...
    # === RUN SUMMARY: Yearly Breakdown (window-aligned with Full Period) ===
    # Uses the same window the Run Summary "Full Period" uses:
    range_full <- paste0(as.character(analysis_start_date), "/", as.character(end(results$portfolio_returns_xts)))
    
    Rp_net_window    <- results$portfolio_returns_xts[range_full]
    Rp_gross_window  <- results$portfolio_returns_xts_gross[range_full]
    Rf_window        <- results$rf_series_xts[range_full]
    Costs_window     <- results$costs_xts[range_full]
    
    # Defensive: stop gracefully if empty
    if (NROW(Rp_net_window) >= 2) {
      # Build annual rows *after* slicing, so first/last years are partial if the window cuts them.
      annual_net       <- xts::apply.yearly(Rp_net_window,   PerformanceAnalytics::Return.cumulative)
      annual_gross     <- xts::apply.yearly(Rp_gross_window, PerformanceAnalytics::Return.cumulative)
      annual_rf        <- xts::apply.yearly(Rf_window,       PerformanceAnalytics::Return.cumulative)
      annual_costs_bps <- xts::apply.yearly(Costs_window,    sum) * 10000
      
      # Assemble a tidy annual table
      yrs <- format(index(annual_net), "%Y")
      annual_df <- data.frame(
        Year                  = yrs,
        Net_Yr_Return_Pct     = 100 * as.numeric(coredata(annual_net)),
        Gross_Yr_Return_Pct   = 100 * as.numeric(coredata(annual_gross)),
        RF_Yr_Return_Pct      = 100 * as.numeric(coredata(annual_rf)),
        Costs_Yr_bps          = as.numeric(coredata(annual_costs_bps)),
        stringsAsFactors = FALSE
      )
      
      # Mark partial first/last years (for transparency)
      first_year <- as.integer(substr(annual_df$Year[1], 1, 4))
      last_year  <- as.integer(substr(annual_df$Year[NROW(annual_df)], 1, 4))
      window_start <- as.Date(start(Rp_net_window))
      window_end   <- as.Date(end(Rp_net_window))
      annual_df$Is_Partial <- FALSE
      # First year partial if window didn’t start on Jan 1 of that year
      annual_df$Is_Partial[1] <- (format(window_start, "%Y") == as.character(first_year)) &&
        (format(window_start, "%m-%d") != "01-01")
      # Last year partial if window didn’t end on Dec 31 of that year
      annual_df$Is_Partial[NROW(annual_df)] <- (format(window_end, "%Y") == as.character(last_year)) &&
        (format(window_end, "%m-%d") != "12-31")
      
      # Reconciliation: product of annual rows should equal the daily product over the same window
      TR_daily <- prod(1 + Rp_net_window, na.rm = TRUE) - 1
      TR_annual <- prod(1 + (annual_df$Net_Yr_Return_Pct/100), na.rm = TRUE) - 1
      delta_bp <- (TR_daily - TR_annual) * 1e4
      
      message(sprintf("[RECON/01] Window: %s / %s",
                      as.character(window_start), as.character(window_end)))
      message(sprintf("[RECON/01] TR(daily)=%.6f | TR(annual_rows)=%.6f | ?=%.1f bps",
                      TR_daily, TR_annual, delta_bp))
      if (abs(delta_bp) > 5) {
        warning("[RECON/01] Annual rows and daily total differ by > 5 bps — window or NA issue.")
      }
      
      # Also compute calendar-time CAGR for the window (the “correct” single-number)
      years_exact <- as.numeric(as.Date(window_end) - as.Date(window_start)) / 365.25
      CAGR_from_daily <- if (years_exact > 0) (1 + TR_daily)^(1/years_exact) - 1 else NA_real_
      message(sprintf("[RECON/01] CAGR(calendar from daily)=%.6f", CAGR_from_daily))
      
      # YTD diagnostics for the current year (both cumulative and annualized run-rate)
      this_year <- format(window_end, "%Y")
      ytd_range <- paste0(this_year, "-01-01/", as.character(window_end))
      Rp_ytd <- Rp_net_window[ytd_range]
      if (NROW(Rp_ytd) >= 1) {
        TR_ytd  <- prod(1 + Rp_ytd, na.rm = TRUE) - 1
        days_ytd <- as.numeric(as.Date(end(Rp_ytd)) - as.Date(start(Rp_ytd)))
        CAGR_ytd <- if (days_ytd > 0) (1 + TR_ytd)^(365.25/days_ytd) - 1 else NA_real_
        message(sprintf("[RECON/01] %s YTD: TR=%.6f | annualized=%.6f | days=%d",
                        this_year, TR_ytd, CAGR_ytd, days_ytd))
      }
      
      # === Write the Yearly Breakdown onto 01_Run_Summary ===
      row0 <- getOption("run_summary_next_row", default = NROW(full_config_df) + 5 + NROW(summary_df) + 3)
      col0 <- getOption("run_summary_yearly_col0", default = 1) 
      
      hdr <- data.frame(
        "Yearly Breakdown (same window as Full Period)" = "",
        check.names = FALSE
      )
      openxlsx::writeData(wb, "01_Run_Summary", hdr, startRow = row0, startCol = col0, colNames = FALSE)
      openxlsx::writeData(wb, "01_Run_Summary", annual_df, startRow = row0 + 1, startCol = col0, rowNames = FALSE)
      
      foot <- data.frame(
        Metric = c("TR (daily window)", "TR (product of annual rows)", "? (bps)", "CAGR (calendar, from daily)"),
        Value  = c(TR_daily, TR_annual, delta_bp, CAGR_from_daily),
        check.names = FALSE
      )
      openxlsx::writeData(wb, "01_Run_Summary", foot, startRow = row0 + 2 + NROW(annual_df), startCol = col0, rowNames = FALSE)
      
      pct_style <- openxlsx::createStyle(numFmt = "0.00%")
      pct_cols <- c("Net_Yr_Return_Pct", "Gross_Yr_Return_Pct", "RF_Yr_Return_Pct")
      for (pc in pct_cols) {
        j <- which(colnames(annual_df) == pc)
        if (length(j) == 1) {
          openxlsx::addStyle(wb, "01_Run_Summary", pct_style, rows = (row0 + 2):(row0 + 1 + NROW(annual_df)), cols = col0 + j - 1, gridExpand = TRUE)
        }
      }
      openxlsx::addStyle(wb, "01_Run_Summary", openxlsx::createStyle(numFmt = "0.00%"),
                         rows = (row0 + 2 + NROW(annual_df)):(row0 + 2 + NROW(annual_df) + 3),
                         cols = col0 + 1, gridExpand = TRUE)
    } else {
      warning("[RUN SUMMARY] Yearly breakdown skipped: insufficient rows in Rp_net_window.")
    }
    # === END Yearly Breakdown ===
  ...
  addWorksheet(wb, "03_Performance_Annual"); tryCatch({
    # --- FIX: Use the exact same window as Tab 01 "Full Period" ---
    Rp_net_window    <- Rp_full_window
    Rp_gross_window  <- results$portfolio_returns_xts_gross[index(Rp_net_window)]
    Rf_window        <- results$rf_series_xts[index(Rp_net_window)]
    Costs_window     <- results$costs_xts[index(Rp_net_window)]
    ...
    # Footer must reconcile to the same window
    Rp_for_footer <- Rp_net_window
    
    merged_annual <- merge(annual_net, annual_gross, annual_rf, annual_costs_bps)
    annual_df <- data.frame(
      Year       = format(index(merged_annual), "%Y"),
      `Net_%`      = coredata(merged_annual[, 1]),
      `Gross_%`    = coredata(merged_annual[, 2]),
      `RiskFree_%` = coredata(merged_annual[, 3]),
      Costs_bps  = coredata(merged_annual[, 4]),
      check.names = FALSE
    )
    
    openxlsx::writeData(wb, "03_Performance_Annual", annual_df, rowNames = FALSE, keepNA = TRUE)
    
    pct_style <- openxlsx::createStyle(numFmt = "0.00%")
    n_rows    <- nrow(annual_df)
    if (n_rows > 0) {
      openxlsx::addStyle(wb, "03_Performance_Annual", pct_style,
                         rows = 2:(n_rows + 1), cols = 2:4, gridExpand = TRUE, stack = TRUE)
    }
    
    # --- Reconciliation footer using canonical values ---
    valid_net <- stats::na.omit(as.numeric(annual_df$`Net_%`))
    TR_from_annual <- if (length(valid_net) > 0) prod(1 + valid_net) - 1 else NA_real_
    years_exact  <- .calc_years_calendar(Rp_for_footer)
    
    CAGR_from_annuals <- if (is.finite(TR_from_annual) && is.finite(years_exact) && years_exact > 0) {
      (1 + TR_from_annual)^(1 / years_exact) - 1
    } else NA_real_
    
    CAGR_from_daily <- CAGR_can # Use the canonical CAGR
    
    recon_df <- data.frame(
      Metric = c("Total Return (from annuals)", "Years (calendar)", "CAGR (from annuals)", "CAGR (from daily)"),
      Value  = c(TR_from_annual, years_exact, CAGR_from_annuals, CAGR_from_daily),
      check.names = FALSE
    )
    
    recon_start_row <- n_rows + 3
    openxlsx::writeData(wb, "03_Performance_Annual", recon_df, startRow = recon_start_row, rowNames = FALSE)
    
    recon_pct_rows <- recon_start_row + c(1, 3, 4)
    openxlsx::addStyle(wb, "03_Performance_Annual", pct_style,
                       rows = recon_pct_rows, cols = 2, gridExpand = TRUE, stack = TRUE)
    
    openxlsx::setColWidths(wb, "03_Performance_Annual", cols = 1:5, widths = "auto")
    
    # Final reconciliation check after building the tab
    delta_bp <- abs(CAGR_can - CAGR_from_daily) * 1e4
    message(sprintf("[RECON/03] CAGR(Canonical) = %.6f | CAGR(03-footer) = %.6f | ? = %.1f bps", CAGR_can, CAGR_from_daily, delta_bp))
    if (delta_bp > 0.1) warning("[RECON/03] 01 vs 03 differ by >0.1 bps — check wiring.")
  ...
  addWorksheet(wb, "14_VIX_Regime_Analysis"); tryCatch({
    if(!is.null(vix_series)) {
      ...
      regime_perf_list <- lapply(levels(regimes), function(reg) {
        ...
        c(CAGR_Pct = cagr_calendar(Rp_regime) * 100, Volatility_Pct = PerformanceAnalytics::StdDev.annualized(Rp_regime, scale=252) * 100, Sharpe_Ratio = PerformanceAnalytics::SharpeRatio.annualized(Rp_regime, Rf=Rf_regime, scale=252), Max_Drawdown_Pct = PerformanceAnalytics::maxDrawdown(Rp_regime) * 100, Num_Days = NROW(Rp_regime))
      })
  ...
  # --- FIX: Add guardrail check before finishing ---
  Rp_full <- results$portfolio_returns_xts
  c01_full <- tryCatch(cagr_calendar(Rp_full[paste0(analysis_start_date, "/", end(Rp_full))]), error = function(e) NA_real_)
  c03_footer <- data_artifact$rpqs_results$`Full Period`$cagr_from_daily_footer
  
  if (!is.null(c01_full) && !is.null(c03_footer) && is.finite(c01_full) && is.finite(c03_footer)) {
    delta_bp <- abs(c01_full - c03_footer) * 10000
    if (delta_bp > 5) {
      warning(sprintf("[RECON_FAIL] Full Period CAGR mismatch > 5 bp. Tab01: %.4f, Tab03_Footer: %.4f", c01_full, c03_footer))
    } else {
      message(sprintf("[RECON_PASS] Full Period CAGR reconciles between Tab01 and Tab03 (delta: %.2f bp).", delta_bp))
    }
  }
  # --- END FIX ---
...
# ============================ SCRIPT ORCHESTRATION (v160.9.20) ===================================

ANALYSIS_PERIODS <- list(
  `Pre-Crisis OOS` = list(start = "2008-01-01", end = "2007-12-31", enabled = TRUE),
  ...
  `YTD 2025`       = list(start = "2025-01-01", end = "2025-11-14", enabled = TRUE)
)
FULL_BACKTEST_START <- "2008-01-01"
FULL_BACKTEST_END   <- "2025-11-14"
...
MASTER_CONTROLS <- list(
  ...
  no_trade_bands = list(
    ...
    snap_mode          = "target",
    vol_tolerance      = 0.01,
    enforce_vol_recenter = FALSE,
    exempt_classes     = c("Cash"),
    respect_hard_rules = TRUE
  ),
  cov_method = "rb_blend",
  cov_lambda = 0.94
)
...
class_map <- .default_class_map(MASTER_CONTROLS$ytd_enhancements$new_asset_classes)
...
data_artifact <- list(run_id = run_id, timestamp_utc = "2025-11-14 12:46:29", user = "balint27",
...
# Add footer cagr to data artifact for later reconciliation
if(!is.null(data_artifact$rpqs_results$`Full Period`)) {
  Rp_full <- data_artifact$results$portfolio_returns_xts
  data_artifact$rpqs_results$`Full Period`$cagr_from_daily_footer <- cagr_calendar(Rp_full)
}
