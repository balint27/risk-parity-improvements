#=============================================================================
# ENHANCED RISK PARITY TRADING SYSTEM - VERSION Z5.3.R (FIXED)
...
# FIXED: Improved logging and visibility of cash allocation decisions
...
cat("\nPart 1 of Fixed Z5.3.R loaded: Enhanced data handling, volatility forecasting, and market indicators\n")
#=============================================================================
# ENHANCED RISK PARITY TRADING SYSTEM - VERSION Z5.3.R (FIXED)
...
#=============================================================================
# REGIME DETECTION WITH GUARANTEED ASSIGNMENT
#=============================================================================
...
# REVISED: Enhanced regime detection with more sensitive thresholds
...
# FIXED: Dramatically reduced turnover threshold to ensure changes are implemented
# This is a critical fix for the zero turnover issue
optimize_for_transaction_costs <- function(prev_weights, current_weights, transaction_costs,
                                           regime, vol_zscore,
                                           base_turnover_threshold = 0.05) { # FIXED: REDUCED from 0.15 to 0.05
  ...
  # FIXED: Lower base threshold dramatically
...
# Helper function to calculate portfolio turnover
...
cat("\nPart 2 of Fixed Z5.3.R loaded: Enhanced regime detection with guaranteed assignment and improved weight optimization\n")
...
calculate_portfolio_performance <- function(
  ...
  for (i in 1:nrow(prices)) {
    ...
    if (date %in% rebalance_dates || i == 1) {
      ...
      # FIXED: Force flag to implement regime changes 
      ...
      if (!is.null(market_data)) {
        ...
        if (nrow(market_subset) >= 30) {
          ...
          # CRITICAL FIX: Track regime changes and force implementation when regime changes
          if (last_regime != "" && last_regime != current_regime) {
            regime_changes <- regime_changes + 1
            
            # CRITICAL FIX: Force implementation of new weights when regime changes
            debug_print(sprintf("IMPLEMENTING REGIME CHANGE: %s → %s", 
                                last_regime, current_regime), important = TRUE)
            
            # Force implementation when regime changes
      ...
      if (!is.null(prev_weights) && length(prev_weights) > 0) {
        ...
        # CRITICAL FIX: Skip transaction cost optimization when force_implementation is TRUE
        if (!force_implementation) {
          ...
          current_weights <- optimize_for_transaction_costs(
            ...
            current_regime, vol_zscore, 0.05  # FIXED: REDUCED turnover threshold to 5%
          )
        } else {
          debug_print("Skipping transaction cost optimization due to regime change", important = TRUE)
        }
      } else {
      ...
      if (i > 1) {
        ...
        # FIXED: Better logging of cash allocation changes
        if (abs(cash_allocation - current_cash) > 0.05 || i == 1) {
          debug_print(sprintf("Significant cash allocation change: %.1f%% → %.1f%%",
                              current_cash * 100, cash_allocation * 100), important = TRUE)
          cat(sprintf("  Cash allocation: %.1f%% (Drawdown: %.1f%%, Vol Z-score: %.1f, Regime: %s)\n", 
                      100 * cash_allocation, 100 * current_drawdown, vol_zscore, current_regime))
        ...
        # CRITICAL FIX: Apply cash allocation to weights and verify it's happening
        if (current_cash > 0) {
          # DEBUG to verify cash allocation is happening
          debug_print(sprintf("Applying cash allocation: %.2f%%", current_cash * 100), important = TRUE)
          
          # Store original weights for logging
          orig_weights <- current_weights
          
          # Scale down all asset weights by (1 - cash)
          for (ticker in names(current_weights)) {
            current_weights[ticker] <- current_weights[ticker] * (1 - current_cash)
            
            # Verify the change with detailed logging
            if (ticker %in% names(orig_weights)) {
              debug_print(sprintf("  Reduced %s weight: %.2f%% → %.2f%%", 
                                  ticker, orig_weights[ticker] * 100, current_weights[ticker] * 100))
            }
          }
        }
      }
...
# FIXED: Improved plot function that correctly displays portfolio performance
plot_performance_comparison <- function(backtest_results, title = "Performance Comparison") {
  if (requireNamespace("ggplot2", quietly = TRUE)) {
    ...
    # FIXED: Ensure regime history is properly plotted
    if (!is.null(backtest_results$strategy$regime_history) && 
      ...
      # FIXED: Also plot weights over time
      weight_history <- backtest_results$strategy$weight_history
      
      # Get top assets by average weight
      avg_weights <- colMeans(weight_history, na.rm = TRUE)
      top_assets <- names(sort(avg_weights[avg_weights > 0.05], decreasing = TRUE))
      
      if (length(top_assets) > 0) {
        # Limit to top 6 assets for clarity
        if (length(top_assets) > 6) {
          top_assets <- top_assets[1:6]
        }
        
        # Extract weight history for top assets
        top_weight_history <- weight_history[, top_assets]
        
        # Convert to long format
        weight_df <- data.frame(
          Date = index(top_weight_history),
          top_weight_history
        )
        
        weight_long <- reshape2::melt(weight_df, id.vars = "Date", 
                                      variable.name = "Asset", value.name = "Weight")
        
        # Create the weight plot
        p_weights <- ggplot(weight_long, aes(x = Date, y = Weight, color = Asset, fill = Asset)) +
          geom_area(position = "stack", alpha = 0.6) +
          theme_minimal() +
          labs(title = "Portfolio Allocation Over Time", y = "Weight", x = "") +
          scale_y_continuous(labels = scales::percent) +
          theme(legend.position = "bottom")
...
cat("\nPart 3 of Fixed Z5.3.R loaded: Performance calculation and cash management with improved plotting\n")
#=============================================================================
# ENHANCED RISK PARITY TRADING SYSTEM - VERSION Z5.3.R (FIXED)
# PART 4: MAIN EXECUTION AND SYSTEM COMPLETION
#=============================================================================
...
# Add rebalance turnover analysis
if (!is.null(backtest_results$strategy$stats$turnover_per_rebalance)) {
  cat(sprintf("\nAverage turnover per rebalance: %.2f%%\n", 
              backtest_results$strategy$stats$turnover_per_rebalance * 100))
}

# FIXED: Add specific analysis on regime changes
if (!is.null(backtest_results$strategy$stats$regime_changes)) {
  cat(sprintf("\nTotal regime changes detected: %d\n", 
              backtest_results$strategy$stats$regime_changes))
}

# FIXED: Analyze current trends
if (trend_following && !is.null(backtest_results$strategy$trend_signals)) {
  current_trends <- tail(backtest_results$strategy$trend_signals, 1)
  uptrend_count <- sum(current_trends > 0, na.rm = TRUE)
  downtrend_count <- sum(current_trends < 0, na.rm = TRUE)
  neutral_count <- sum(current_trends == 0, na.rm = TRUE)
  
  cat("\n========== CURRENT TREND ANALYSIS ==========\n")
  cat(sprintf("Assets in uptrend: %d (%.1f%%)\n", 
              uptrend_count, uptrend_count / ncol(current_trends) * 100))
  cat(sprintf("Assets in downtrend: %d (%.1f%%)\n", 
              downtrend_count, downtrend_count / ncol(current_trends) * 100))
  cat(sprintf("Assets in neutral trend: %d (%.1f%%)\n", 
              neutral_count, neutral_count / ncol(current_trends) * 100))
  
  # Show specific trend status for main assets
  cat("\nCurrent trends for key assets:\n")
  for (ticker in tickers) {
    if (ticker %in% colnames(current_trends)) {
      trend_value <- as.numeric(current_trends[, ticker])
      trend_label <- ifelse(trend_value > 0, "UPTREND", 
                            ifelse(trend_value < 0, "DOWNTREND", "NEUTRAL"))
      cat(sprintf("  %s: %s\n", ticker, trend_label))
    }
  }
}
...
cat(sprintf("Strategy Total Return: %.2f%% (Equal-Weight: %.2f%%)\n", 
            backtest_results$strategy$stats$total_return * 100,
            backtest_results$equal_weight$stats$total_return * 100))
cat(sprintf("Strategy Annualized Return: %.2f%% (Equal-Weight: %.2f%%)\n", 
            backtest_results$strategy$stats$mean_return * 100,
            backtest_results$equal_weight$stats$mean_return * 100))
cat(sprintf("Strategy Sharpe Ratio: %.2f (Equal-Weight: %.2f)\n", 
            backtest_results$strategy$stats$sharpe,
            backtest_results$equal_weight$stats$sharpe))
cat(sprintf("Strategy Max Drawdown: %.2f%% (Equal-Weight: %.2f%%)\n", 
            backtest_results$strategy$stats$max_drawdown * 100,
            backtest_results$equal_weight$stats$max_drawdown * 100))

if (!is.null(backtest_results$spy)) {
  cat(sprintf("SPY Total Return: %.2f%%\n", 
              backtest_results$spy$stats$total_return * 100))
  cat(sprintf("SPY Sharpe Ratio: %.2f\n", 
              backtest_results$spy$stats$sharpe))
}

cat("\n========== ENHANCED RISK PARITY STRATEGY COMPLETE ==========\n")
cat("Thank you for using the Enhanced Risk Parity System Z5.3.R (FIXED VERSION)\n")
cat("Current Date and Time (UTC):", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "\n")
cat("Current User's Login:", "balint27ni", "\n")