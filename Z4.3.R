...
optimize_risk_parity <- function(returns, vol_forecasts, target_risk_weights = NULL,
  ...
  # Ensure covariance matrix has no NA values
  if (any(is.na(cov_matrix))) {
    cat("WARNING: NA values in covariance matrix, replacing with zeros\n")
    cov_matrix[is.na(cov_matrix)] <- 0
    # Add a small positive value to diagonal for stability
    diag(cov_matrix) <- diag(cov_matrix) + 1e-6
  }
  
  # Extract volatility forecasts with enhanced error handling
  vols <- try({
    if (is.data.frame(vol_forecasts)) {
      v <- vol_forecasts$forecasted_vol[asset_names]
      # Debug information
      ...
      cat("WARNING: Invalid volatility forecasts, using default values\n")
      rep(0.15, n_assets)  # Default value
    }
  }, silent = TRUE)
  
  # Fallback if extraction failed
  if (inherits(vols, "try-error") || length(vols) != n_assets) {
    cat("ERROR extracting volatilities, using default values\n")
    vols <- rep(0.15, n_assets)
    names(vols) <- asset_names
  }
  
  # Check for invalid vols and replace with default values
  if (any(is.na(vols)) || any(vols <= 0)) {
    cat("WARNING: Invalid volatility values detected, replacing with median\n")
    valid_vols <- vols[!is.na(vols) & vols > 0]
    if (length(valid_vols) > 0) {
      median_vol <- median(valid_vols)
    } else {
      median_vol <- 0.15  # Default if no valid values
    }
  ...
  # Fixed to handle NA values and other issues
  risk_deviation <- function(w) {
    ...
    port_vol <- tryCatch({
      sqrt(max(1e-10, as.numeric(t(w) %*% cov_matrix %*% w)))
    }, error = function(e) {
      cat("Error in volatility calc:", e$message, "\n")
    ...
    # Calculate risk contribution for each asset with safety checks
    marginal_contrib <- tryCatch({
      as.vector(cov_matrix %*% w) / port_vol
    }, error = function(e) {
      cat("Error in marginal contribution calc:", e$message, "\n")
      return(w * vols / port_vol)  # Simplified fallback
    })
    
    risk_contrib <- w * marginal_contrib
    
    # Calculate proportional risk contribution
    sum_risk <- sum(risk_contrib)
    if (sum_risk > 0) {
      risk_proportion <- risk_contrib / sum_risk
    } else {
      return(1e10)  # Invalid risk contributions
    }
  ...
  # Starting point - inverse volatility weights with safety checks
  ...
  opt_result <- tryCatch({
    # Check if optimization function works properly
    test_result <- risk_deviation(w0)
    
    if (is.na(test_result) || is.infinite(test_result)) {
      cat("WARNING: Optimization function returns invalid value, using inverse volatility weights\n")
      stop("Optimization error")  # Force fallback
    }
  ...
  # Ensure weights satisfy constraints and have no NAs
...
backtest_strategy <- function(prices, returns = NULL, market_data = NULL,
  ...
  for (i in 1:length(rebalance_dates)) {
    ...
    # Ensure weights are properly named and sized
    if (is.null(names(new_weights)) || length(new_weights) != length(asset_names)) {
      ...
      } else if (length(new_weights) == n_assets) {
        # If length matches but no names, assume order matches
        fixed_weights <- new_weights
      }
      new_weights <- fixed_weights
    }
    
    # Handle NAs in weights
    if (any(is.na(new_weights))) {
      cat("WARNING: NA values in weights, replacing with zeros\n")
      new_weights[is.na(new_weights)] <- 0
    }
    ...
    # FIXED: Store weights and metrics for holding period with proper numeric indexing
    weight_assignment_count <- 0
    for (holding_date in holding_dates) {
      holding_date_string <- as.character(holding_date)
      
      # Find position of this date in common_dates (which is what we used to create the matrix)
      holding_idx <- which(common_dates == holding_date)
      
      if (length(holding_idx) > 0) {
        # DEBUG info
        cat(sprintf("  Processing date %s (index: %d)\n", holding_date_string, holding_idx))
        
        # Use numeric row indexing instead of string-based
        for (j in 1:n_assets) {
          weights_matrix[holding_idx, j] <- new_weights[j]
        }
        
        # Verify weights were assigned
        row_sum <- sum(abs(weights_matrix[holding_idx,]))
        cat(sprintf("    Assigned weights, sum: %.4f\n", row_sum))
        ...
        cat(sprintf("  WARNING: Date %s not found in common_dates\n", holding_date_string))
      }
    }
  ...
  # Add this after the loop that processes all rebalance dates:
  # Verify weights were actually assigned
  non_zero_weight_dates <- sum(rowSums(abs(weights_matrix)) > 1e-6)
  cat(sprintf("\nVerification: %d out of %d dates have non-zero weights (%.1f%%)\n", 
              non_zero_weight_dates, nrow(weights_matrix), 
              100 * non_zero_weight_dates / nrow(weights_matrix)))
  
  if (non_zero_weight_dates == 0) {
    cat("CRITICAL ERROR: No weights were assigned! Check date indexing.\n")
    # Debug: print sample of weights_matrix row names and common_dates
    cat("Sample row names from weights_matrix:\n")
    print(head(rownames(weights_matrix), 3))
    cat("Sample dates from common_dates:\n")
    print(head(common_dates, 3))
  }
